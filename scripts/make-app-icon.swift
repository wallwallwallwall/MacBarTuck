...
let output = CommandLine.arguments.dropFirst().first.map(URL.init(fileURLWithPath:))
    ?? root.appendingPathComponent("MacBarTuck/Resources/MacBarTuck.icns")
let workDirectory = root.appendingPathComponent("work/AppIcon", isDirectory: true)
let iconset = workDirectory.appendingPathComponent("MacBarTuck.iconset", isDirectory: true)
...
func writePNG(_ image: NSImage, pixels: Int, to url: URL) throws {
    guard let representation = NSBitmapImageRep(
        ...
        bytesPerRow: 0,
        bitsPerPixel: 0
    ) else { throw NSError(domain: "MacBarTuckIcon", code: 1) }
    ...
    guard let data = representation.representation(using: .png, properties: [:]) else {
        throw NSError(domain: "MacBarTuckIcon", code: 2)
    }
