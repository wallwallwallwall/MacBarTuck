...
- 菜单项：搜索项目，用“显示方式”菜单选择自动、始终显示或收起。选择“收起”只保存规则，不会在逐项配置时反复移动菜单栏；完成后点击右上角“应用显示方式”统一执行一次。
...
当前基础测试集包含 436 项规则、菜单身份、图标选择、权限、交互策略、应用入口、托盘外观及双语资源检查。`bash scripts/run-refresh-tests.sh` 另执行 18 项 Store 调度检查及诊断日志轮换检查，其中包含“相同扫描复用原项目对象”“项目已经收纳时重复应用不得调整隐藏分隔区或再次拖动”等回归。系统授权及激活策略测试主要使用替身，另覆盖实际 AppKit 重复设置策略的返回行为；不修改本机授权。这些测试不能代替真实收纳和点击验收。
...
产物位于 `dist/MacBarTuck-0.1.16.dmg`，并附带 SHA-256 文件。
...
- 临时签名的应用在更新代码后可能需要重新授予系统权限。0.1.16 已在 Apple Silicon、macOS 27.0（26A428）真机验证菜单项发现，并用 macOS 27 SDK 完成核心测试编译；macOS 15、显示器热插拔和完整 Xcode 27 应用构建仍未完成实机或工具链验收。
