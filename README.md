...
- 点击托盘中的图标，继续使用原应用菜单或弹窗；macOS 27 优先直接调用原生菜单，不再先移动整排菜单栏项目。
- 托盘左侧直接显示当前收纳数量；正常收纳、临时显示和重新收纳使用不同颜色与文字区分。
...
- macOS 15.0 或更高版本；包含 macOS 27 及后续版本的专用保守兼容路径。
- 辅助功能权限：识别菜单栏控件并执行原本的点击动作。
- 屏幕录制权限：只截取菜单栏图标的小区域，用于托盘显示。

## 安装

1. 打开 `MacBarTuck-0.1.23.dmg`。
...
不要同时运行多个菜单栏整理工具，例如 iBar、Bartender 或 Ice。它们会竞争同一组系统菜单栏项目。
...
- 菜单项：搜索项目，用“显示方式”菜单选择自动、始终显示或收起。选择“收起”只保存规则；完成后点击右上角“应用显示方式”统一执行一次。macOS 27 使用系统菜单栏 Assessment Mode 按应用隐藏项目，不再创建遮罩或透明占位项；macOS 15/26 继续使用原有系统菜单栏布局路径。
- 托盘左侧显示“已收纳 N 项”；项目若需要先移回真实菜单栏才能打开，会标记为“临时显示”。点击右侧琥珀色重新收纳按钮，可一次收回所有临时显示项，避免每次打开菜单都来回重排。
- 顶部“启用收纳”开关控制是否移动菜单项。“全部显示”会暂停收纳并恢复原图标。
- 收纳完成后，MacBarTuck 自身入口以及控制中心、Wi-Fi、时钟、录屏/定位等系统安全项目仍会保留；这些系统图标不计入第三方应用的收纳结果。
...
当前独立测试集覆盖规则、菜单身份、图标选择、权限、交互策略、应用入口、托盘外观、macOS 27 Assessment Mode、旧版遮罩兼容及双语资源。`bash scripts/run-refresh-tests.sh` 另覆盖 Store 调度、刷新隔离、权限撤销、重复应用、恢复和诊断日志轮换。系统授权及激活策略测试主要使用替身，另有双屏真实菜单栏探针；这些测试不能代替所有系统版本上的真实点击验收。
...
产物位于 `dist/MacBarTuck-0.1.23.dmg`，并附带 SHA-256 文件。
...
0.1.13 修正实机托盘与预览不一致：真实无边框面板固定使用深色表面，正常状态显示“已收纳 N 项”或 `N Tucked`；应用图标调整到更紧凑的 26 pt，系统图标为 20 pt。内容能完整放下时不创建滚动视图，避免右侧出现无意义的小横条。

0.1.14 将规则配置和菜单栏移动拆开：选择“收起”只记录待应用规则，完成后在菜单项页点击一次“应用显示方式”。主操作不再藏在通用页，逐项配置也不会连续触发布局。手动应用忽略 macOS 会话中可能长期残留的鼠标按键状态，后台自动布局仍会避开真实按压，避免按钮无响应或持续重试造成双屏图标晃动。

0.1.15 稳定刷新身份与应用图标：相同项目继续复用原对象和同一个图像实例，无真实变化时不重新发布列表；登录项 helper 会解析到外层应用名称和图标，并支持 `CFBundleIconFile`、`CFBundleIconName`、`CFBundleIconFiles` 与 `CFBundleIcons`。输入法使用真实菜单栏图像或键盘符号，不再显示通用应用占位图标。针对 macOS 27 的复合菜单栏宿主，扫描只把单个状态项尺寸的 WindowServer 窗口交给 AX 合并，移动与恢复使用逐项 Accessibility 路径并保留独立恢复位置。

0.1.16 修复 macOS 27 升级后列表为空：扫描优先读取每个应用的 `AXExtrasMenuBar`，将存在但为空的 `AXTitle` 继续回退到描述或应用名称，并接受菜单栏内部有垂直留白的图标。macOS 27 不再用动态 WindowServer 标题生成项目身份；AX 项目改用应用 bundle 与应用内顺序形成稳定 ID，CPU、网速等实时文字变化不会重建列表。无标题且不可操作的 `MenuBarAgent` 宿主不会作为重复项目显示，长标题的真实菜单栏状态项仍会保留。

0.1.17 修复 macOS 27 启用收纳后原图标仍在、点击时整排图标晃动的问题：新系统不再拖动复合菜单栏宿主，而是在主屏和扩展屏上为每个已确认项目创建固定遮罩。遮罩只在位置真实变化时原地更新，重复刷新或重复应用不会重建窗口。托盘左键优先直接执行原项目的辅助功能动作，只有原生动作失败时才短暂揭开对应位置并执行点击；退出、关闭收纳、项目消失或录屏权限失效时会同步移除遮罩。

0.1.18 修复 macOS 27 菜单栏遮罩显示为黑色矩形的问题：每块屏幕只截取一次真实菜单栏背景，再按项目位置生成无前景补片，不再叠加深色系统材质。无变化刷新不会重新截图或重建窗口；切换 Space、唤醒和显示器变化后只更新原窗口的背景图。重抓时会排除 MacBarTuck 自身遮罩，截图暂时失败则保留上一张有效背景，避免退回黑框。

0.1.19 取消 macOS 27 的逐项遮罩。已收纳项目会被一次性排列到原生分隔项左侧，再由两个有界透明分隔项推入系统溢出区；保留项目和 MacBarTuck 图标在右侧连续排列，因此不再保留原图标槽位，也不会出现黑框或一段段透明空洞。移动结果需要连续两次读取到相邻位置才提交，重复应用和普通刷新保持纯只读；关闭收纳或退出应用会恢复应用启动前的顺序。macOS 15/26 仍沿用原有兼容路径。

0.1.20 修复 macOS 27 收纳后菜单栏中间仍有大段空白的问题。透明状态项即使不可见仍会占据真实宽度，因此新版本不再创建分隔项，也不再拖动整排图标；改用系统 Assessment Mode 按菜单栏宿主应用控制可见性，隐藏后由 macOS 立即回收位置。重复刷新不会替换相同配置，临时打开托盘项目时会先显示该应用的整组菜单项，重新收纳或退出后恢复对应状态。macOS 15/26 仍使用原兼容路径。

0.1.21 修复 macOS 27 点击已收纳应用后只临时显示图标、却没有弹出应用菜单的问题。Assessment Mode 重新显示应用时可能重建状态项；新版本不再信任收纳前保存的旧辅助功能元素，而是在显示完成后重新解析当前元素，再执行菜单点击。

0.1.22 修复 macOS 27 上相邻菜单栏项目因辅助功能区域重叠而被跨应用合并的问题。扫描器现在只允许同一进程的 AX 项目互相匹配；跨进程匹配仅用于尚未绑定辅助功能元素的旧系统 WindowServer 种子，避免 Tailscale、Snipaste、钉钉等项目出现名称与彩色图标串用。

0.1.23 修复启动刷新时的重复 ID 崩溃。断开扩展屏后，普通应用遗留在屏外的 `AXMenuBar` 曾被误判为隐藏状态项；当 helper 又解析到同一个外层应用时，会产生重复 `ax|bundle|0`，下一次刷新触发 Swift 字典断言。新版本只从普通应用的 `AXExtrasMenuBar` 接纳未匹配项目，并在刷新边界按首次出现顺序清理异常重复 ID，避免列表、图标和 SwiftUI 视图复用串位。
...
- macOS 27 路径动态调用未公开的系统 Assessment Mode 接口，适合开源侧载分发，不适合提交 Mac App Store；系统升级后仍可能需要继续适配。
...
- 临时签名的应用在更新代码后可能需要重新授予系统权限。0.1.23 使用 Xcode 27 和 macOS 27 SDK 构建；macOS 15/26 由兼容策略回归覆盖，但本轮没有对应老系统真机。
