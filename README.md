...
当前基础测试集包含 421 项规则、菜单身份、图标选择、权限、交互策略、应用入口、托盘外观及双语资源检查。`bash scripts/run-refresh-tests.sh` 另执行 18 项 Store 调度检查及诊断日志轮换检查，其中包含“相同扫描复用原项目对象”“项目已经收纳时重复应用不得调整隐藏分隔区或再次拖动”等回归。系统授权及激活策略测试主要使用替身，另覆盖实际 AppKit 重复设置策略的返回行为；不修改本机授权。这些测试不能代替真实收纳和点击验收。
...
产物位于 `dist/MacBarTuck-0.1.15.dmg`，并附带 SHA-256 文件。
...
- 临时签名的应用在更新代码后可能需要重新授予系统权限。macOS 15、macOS 27 RC 和显示器热插拔尚未完成实机验收；0.1.15 已在 macOS 26.6.2、Xcode 26.6、macOS SDK 26.5 上完成向前兼容编译与策略回归，但仍需 Xcode 27 RC / macOS 27 RC 环境完成最终验收。
