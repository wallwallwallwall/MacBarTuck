...
当前基础测试集包含 371 项规则、菜单身份、图标选择、权限、交互策略、应用入口及双语资源检查。`bash scripts/run-refresh-tests.sh` 另执行 10 项 Store 调度检查及诊断日志轮换检查。系统授权及激活策略测试主要使用替身，另覆盖实际 AppKit 重复设置策略的返回行为；不修改本机授权。这些测试不能代替真实收纳和点击验收。
...
产物位于 `dist/MacBarTuck-0.1.11.dmg`，并附带 SHA-256 文件。
