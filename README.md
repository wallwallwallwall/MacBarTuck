...
- 菜单项：搜索项目，用“显示方式”菜单选择自动、始终显示或收起。选择“收起”只保存规则；完成后点击右上角“应用显示方式”统一执行一次。macOS 27 使用稳定遮罩盖住原图标，macOS 15/26 继续使用系统菜单栏布局路径。
...
当前独立测试集包含 483 项规则、菜单身份、图标选择、权限、交互策略、应用入口、托盘外观、双屏遮罩及双语资源检查。`bash scripts/run-refresh-tests.sh` 另覆盖 55 个带独立错误码的 Store 调度、刷新隔离、权限撤销、Space/唤醒背景更新和遮罩生命周期断言，并检查诊断日志轮换。系统授权及激活策略测试主要使用替身，另有双屏真实窗口探针；不修改本机授权。这些测试不能代替所有系统版本上的真实点击验收。
...
产物位于 `dist/MacBarTuck-0.1.18.dmg`，并附带 SHA-256 文件。
...
- 临时签名的应用在更新代码后可能需要重新授予系统权限。0.1.18 使用 Xcode 27 和 macOS 27 SDK 构建；macOS 15/26 由兼容策略回归覆盖，但本轮没有对应老系统真机。
