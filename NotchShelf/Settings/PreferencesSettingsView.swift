...
struct PreferencesSettingsView: View {
    ...
    private var behaviorSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            NotchShelfSectionTitle("行为", detail: "修改后立即生效")
            NotchShelfSurface(padding: 0) {
                VStack(spacing: 0) {
                    ...
                    preferenceToggle(
                        symbol: "power",
                        title: "登录时启动",
                        detail: "登录 macOS 后自动运行 NotchShelf",
                        isOn: Binding(
            ...
            if let error = launchAtLogin.errorMessage {
                Label(error, systemImage: "exclamationmark.triangle.fill")
                    .font(.system(size: 10))
                    .foregroundStyle(NotchShelfTheme.warning)
            }
        }
    }
    ...
    private var operationSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            NotchShelfSectionTitle("菜单栏操作", detail: "恢复操作不会删除你的规则")
            NotchShelfSurface {
    ...
    private var separator: some View {
        Divider().overlay(NotchShelfTheme.stroke).padding(.leading, 52)
    }
