...
final class MenuBarItemStore: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.notchshelf.app", category: "items")
    ...
    /// Items currently in NotchShelf's off-screen staging area but absent
    ...
    func refresh() {
        ...
        // items do not remain beside NotchShelf while avoiding a drag during
    ...
    func prepareForUIPreview() {
        ...
        items = samples.enumerated().map { index, sample in
            MenuBarItem(
                ...
                title: sample.1,
                ownerName: sample.2,
                bundleIdentifier: sample.5 ? "com.apple.controlcenter" : "com.notchshelf.preview",
