...
struct NotchShelfSurface<Content: View>: View {
...
struct NotchShelfSectionTitle: View {
    let title: String
    let detail: String?
...
struct NotchShelfStatusPill: View {
    let title: String
    let color: Color
        ...
        Group {
            if let image = item.displayImage {
                Image(nsImage: image)
                    ...
                    .resizable()
                    .scaledToFit()
                    .foregroundStyle(item.usesTemplateIcon && colorScheme == .dark ? NotchShelfTheme.primaryText : Color.primary)
            } else {
...
extension MenuItemRule {
    ...
    var tint: Color {
        switch self {
        case .automatic: NotchShelfTheme.accent
        case .alwaysVisible: NotchShelfTheme.success
        case .alwaysHidden: NotchShelfTheme.warning
        }
    }
}
